...
def compute_geometric_mean(
    ...
    if any(score is None for score in [entailed_score, coverage_score, decontextualization_score]):
        logger.debug(
    ...
    for score_name, score_val in [
        ...
        if not (0.0 <= score_val <= 1.0):
            logger.warning(f"Score {score_name}={score_val} is outside valid range [0.0, 1.0]")
            return None
...
def meets_quality_threshold(
    ...
    """
    geomean = compute_geometric_mean(entailed_score, coverage_score, decontextualization_score)
...
def should_allow_concept_linking(
    ...
    if any(score is None for score in [entailed_score, coverage_score, decontextualization_score]):
        logger.debug(f"Concept linking rejected: null scores present for {relationship_type}")
        return False
    
    # For MENTIONS_CONCEPT, allow even if below threshold (as long as no nulls)
    if relationship_type == "MENTIONS_CONCEPT":
        logger.debug(f"Allowing {relationship_type} relationship despite potential low scores")
        return True
    ...
    logger.warning(f"Unknown relationship type: {relationship_type}, requiring quality threshold")
    return meets_quality_threshold(
        entailed_score, coverage_score, decontextualization_score, quality_threshold
    )
