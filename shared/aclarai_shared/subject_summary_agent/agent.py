...
class SubjectSummaryAgent:
    ...
    def _generate_agentic_content(
        ...
        try:
            ...
            if not self.definition_writer_agent:
                raise RuntimeError("DefinitionWriterAgent not initialized")
            summary_paragraph = self.definition_writer_agent.generate_definition(
                subject_name, concepts, context
            )

            # Delegate to ConceptBlurbAgent for each concept
            if not self.concept_blurb_agent:
                raise RuntimeError("ConceptBlurbAgent not initialized")
            concept_blurbs = {}
            ...
            if not self.common_threads_agent:
                raise RuntimeError("CommonThreadsAgent not initialized")
