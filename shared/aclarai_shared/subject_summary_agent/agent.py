...
class SubjectSummaryAgent:
    ...
    def get_cluster_assignments(self) -> Optional[Dict[str, int]]:
        ...
        try:
            assignments = self.clustering_job.get_cluster_assignments()
            if assignments:
                logger.debug(
    ...
    def retrieve_shared_claims(self, concept_names: List[str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            WHERE c.name IN ['{concept_names_str}']
    ...
    def search_web_context(self, concepts: List[str]) -> Optional[str]:
        ...
        try:
            ...
            result = self.web_search_tool.call(query=search_query)
    ...
    def generate_subject_content(
        ...
        """
        Generate the Markdown content for a subject file using LLM.

        This follows the format specified in docs/arch/on-writing_vault_documents.md
        for the Subject Summary Agent.
        ...
        """
        if self.llm:
            # Use LLM to generate intelligent subject content
            try:
                content = self._generate_llm_content(subject_name, concepts, context)
                if content:
                    return content
            except Exception as e:
                logger.warning(
                    f"LLM generation failed for subject '{subject_name}', falling back to template: {e}",
                    extra={
                        "service": "aclarai",
    ...
    def _generate_llm_content(
        ...
        """
        Generate subject content using LLM with retrieved context.
        ...
        prompt_parts = [
            f"Generate a comprehensive subject summary page for: {subject_name}",
            "",
            "This subject page should summarize and contextualize a group of related concepts.",
            "",
            "The page should follow this format:",
            f"## Subject: {subject_name}",
            "",
            "[A clear summary paragraph explaining what this subject encompasses]",
            "",
            "### Included Concepts",
            "[List of concepts with short internal blurbs]",
            "",
            "### Common Threads",
            "[Summary of shared topics from claims and summaries]",
            "",
