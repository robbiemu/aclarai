...
class TestShouldAllowConceptLinking:
    ...
    def test_allow_contradicts_concept_above_threshold(self):
        ...
        result = should_allow_concept_linking(
            0.9, 0.8, 0.85, 0.7, "CONTRADICTS_CONCEPT"
        )
        assert result is True
...
class TestIntegrationWithTypicalValues:
    ...
    def test_high_quality_claim_scenario(self):
        ...
        assert (
            should_allow_vault_promotion(entailed, coverage, decontex, threshold)
            is True
        )
        assert (
            should_include_in_summaries(entailed, coverage, decontex, threshold) is True
        )
    ...
    def test_low_quality_claim_scenario(self):
        ...
        assert (
            should_include_in_summaries(entailed, coverage, decontex, threshold)
            is False
        )
