...
class TestCoverageAgentIntegration:
    ...
    def test_coverage_evaluation_end_to_end_workflow(
        ...
        with patch.object(
            graph_service, "update_relationship_score"
        ) as mock_update_score:
            mock_update_score.return_value = True

            score_stored = graph_service.update_coverage_score(
                claim_id, source_id, score
            )
            assert score_stored is True
            mock_update_score.assert_called_once_with(
                claim_id, source_id, "coverage_score", 0.75
            )
        ...
        with (
            patch.object(markdown_service, "_atomic_write") as mock_atomic_write,
...
"""
    ...
    def test_coverage_evaluation_handles_null_score(
        self, mock_load_prompt, coverage_agent, graph_service, markdown_service
    ):
        ...
        with patch.object(
            ...
            score_stored = graph_service.update_coverage_score(
                "claim_123", "source_456", None
            )
            assert score_stored is True
            mock_update_score.assert_called_once_with(
                "claim_123", "source_456", "coverage_score", None
            )
...
"""
    ...
    def test_coverage_agent_tool_integration(
        self, mock_tool_factory, mock_llm, mock_config
    ):
        ...
        with patch(
            ...
            mock_tool_factory.get_tools_for_agent.assert_called_once_with(
                "coverage_agent"
            )
