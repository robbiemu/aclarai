...
class TestClaimEvaluationGraphServiceElementsAndOmits:
    ...
    def test_create_element_nodes_and_omits_relationships_success(self, graph_service, mock_driver):
    ...
    def test_create_element_nodes_and_omits_relationships_empty_elements(self, graph_service, mock_driver):
        """Test handling of empty omitted elements list."""
        result = graph_service.create_element_nodes_and_omits_relationships("claim123", [])
        
        assert result is True
        # Should not attempt any database operations
        mock_driver.session.assert_not_called()

    def test_create_element_nodes_and_omits_relationships_partial_creation(self, graph_service, mock_driver):
    ...
    def test_create_element_nodes_and_omits_relationships_database_error(self, graph_service, mock_driver):
    ...
    def test_update_coverage_score_delegates_to_update_relationship_score(self, graph_service):
        ...
        with patch.object(graph_service, 'update_relationship_score') as mock_update:
            ...
            assert result is True
            mock_update.assert_called_once_with("claim123", "block456", "coverage_score", 0.85)
    ...
    def test_update_coverage_score_with_null_score(self, graph_service):
        ...
        with patch.object(graph_service, 'update_relationship_score') as mock_update:
            ...
            assert result is True
            mock_update.assert_called_once_with("claim123", "block456", "coverage_score", None)
