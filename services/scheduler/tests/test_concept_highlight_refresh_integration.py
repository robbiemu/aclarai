...
class TestConceptHighlightRefreshIntegration:
    ...
    def setup_method(self):
        ...
        self.config.scheduler.jobs.trending_topics.count = 3
    ...
    def _create_sample_neo4j_data(self):
        ...
        window_days = self.config.scheduler.jobs.trending_topics.window_days
        recent_timestamp = (
            now - timedelta(days=2)
        ).isoformat()  # Within current window (0-7 days)
        old_timestamp = (
            now - timedelta(days=window_days + 2)
        ).isoformat()  # Within comparison window (7-14 days)

        # It's better to use a single query to ensure atomicity
        ...
        CREATE (claim5:Claim {text: 'claim 5'}) // FIX: Create claim5
        CREATE (claim6:Claim {text: 'claim 6'}) // FIX: Create claim6

        // Create Relationships for PageRank
        CREATE (ml)-[:RELATED_TO]->(dl)
        CREATE (dl)-[:RELATED_TO]->(nn)
        CREATE (dl)-[:RELATED_TO]->(ai)
        ...
        CREATE (claim1)-[:MENTIONS_CONCEPT {created_at: $recent_ts, classified_at: $recent_ts}]->(nlp)
        CREATE (claim2)-[:MENTIONS_CONCEPT {created_at: $recent_ts, classified_at: $recent_ts}]->(nlp)
        CREATE (claim5)-[:MENTIONS_CONCEPT {created_at: $old_ts, classified_at: $old_ts}]->(nlp)

        // CV: 1 recent mention, 2 old mentions for comparison
        CREATE (claim3)-[:MENTIONS_CONCEPT {created_at: $recent_ts, classified_at: $recent_ts}]->(cv)
        CREATE (claim4)-[:MENTIONS_CONCEPT {created_at: $old_ts, classified_at: $old_ts}]->(cv)
        CREATE (claim6)-[:MENTIONS_CONCEPT {created_at: $old_ts, classified_at: $old_ts}]->(cv)

        // Add SUPPORTS_CONCEPT relationships to ensure all required types exist
        CREATE (claim1)-[:SUPPORTS_CONCEPT {created_at: $recent_ts, classified_at: $recent_ts}]->(nlp)
        CREATE (claim3)-[:SUPPORTS_CONCEPT {created_at: $recent_ts, classified_at: $recent_ts}]->(cv)
        """

        self.neo4j_manager.execute_query(
            setup_query,
            parameters={"recent_ts": recent_timestamp, "old_ts": old_timestamp},
        )
