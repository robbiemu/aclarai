...
from aclarai_scheduler.concept_summary_refresh import ConceptSummaryRefreshJob
...
class TestConceptSummaryRefreshJob:
    ...
    def test_run_job_success(self):
        ...
        with patch("aclarai_scheduler.concept_summary_refresh.ConceptSummaryAgent") as mock_agent_class:
            with patch("aclarai_scheduler.concept_summary_refresh.Neo4jGraphManager") as mock_neo4j_class:
                ...
                # Mock the methods
                job._get_canonical_concepts = Mock(return_value=["Concept A", "Concept B"])
    ...
    def test_run_job_with_skipped_concepts(self):
        ...
        with patch("aclarai_scheduler.concept_summary_refresh.ConceptSummaryAgent") as mock_agent_class:
            with patch("aclarai_scheduler.concept_summary_refresh.Neo4jGraphManager") as mock_neo4j_class:
                ...
                job._get_canonical_concepts = Mock(return_value=["Concept A", "Concept B", "Concept C"])
    ...
    def test_run_job_with_errors(self):
        ...
        with patch("aclarai_scheduler.concept_summary_refresh.ConceptSummaryAgent") as mock_agent_class:
            with patch("aclarai_scheduler.concept_summary_refresh.Neo4jGraphManager") as mock_neo4j_class:
                ...
                assert result["success"] is True  # Still succeeds if some concepts are processed
                assert result["concepts_processed"] == 1
                assert result["concepts_updated"] == 1
                assert result["concepts_skipped"] == 0
                assert result["errors"] == 1
                assert len(result["error_details"]) == 1
                assert "Concept 'Concept B': Processing error" in result["error_details"]
    ...
    def test_run_job_no_concepts(self):
        ...
        with patch("aclarai_scheduler.concept_summary_refresh.ConceptSummaryAgent") as mock_agent_class:
            with patch("aclarai_scheduler.concept_summary_refresh.Neo4jGraphManager") as mock_neo4j_class:
                ...
                assert result["success"] is True  # Should still succeed with no concepts
    ...
    def test_get_canonical_concepts_success(self):
        ...
        with patch("aclarai_scheduler.concept_summary_refresh.ConceptSummaryAgent"):
            with patch("aclarai_scheduler.concept_summary_refresh.Neo4jGraphManager") as mock_neo4j_class:
    ...
    def test_get_canonical_concepts_failure(self):
        ...
        with patch("aclarai_scheduler.concept_summary_refresh.ConceptSummaryAgent"):
            with patch("aclarai_scheduler.concept_summary_refresh.Neo4jGraphManager") as mock_neo4j_class:
                ...
                mock_neo4j = Mock()
                mock_neo4j_class.return_value = mock_neo4j
                mock_neo4j.get_driver.side_effect = Exception("Database connection failed")
