...
class ConceptSummaryRefreshJob:
        ...
        try:
            ...
            for concept_name in concepts:
                try:
                    ...
                    job_stats["errors"] += 1
                    job_stats["error_details"].append(f"Concept '{concept_name}': {str(e)}")
            
            # Job succeeds if we processed at least one concept without catastrophic failure
            job_stats["success"] = job_stats["concepts_processed"] > 0 or len(concepts) == 0
    ...
    def _get_canonical_concepts(self) -> List[str]:
        ...
        try:
            with self.neo4j_manager.get_driver() as driver:
    ...
    def _process_concept(self, concept_name: str) -> tuple[bool, bool]:
        ...
        try:
            ...
            if self.config.concept_summaries.skip_if_no_claims:
                if not self._concept_has_claims(concept_name):
                    self.logger.info(
            ...
            result = self.concept_summary_agent.process_concept(concept_name)
