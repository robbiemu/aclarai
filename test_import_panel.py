...
class TestImportStatusTracker:
    ...
    def test_map_plugin_status_to_ui(self):
        ...
        assert ImportStatusTracker._map_plugin_status_to_ui(
...
class TestRealPluginOrchestrator:
    ...
    def test_process_text_file(self):
        ...
        try:
            ...
            assert updated_tracker.import_queue[0]["status"] in ["✅ Imported", "⚠️ Fallback"]
    ...
    def test_duplicate_file_handling(self):
        ...
        try:
            ...
            queue_display1, summary_display1, tracker1 = real_plugin_orchestrator(
                test_file, tracker
            )
            queue_display2, summary_display2, tracker2 = real_plugin_orchestrator(
                test_file, tracker1
            )
            
            # Second file should be skipped as duplicate
            assert len(tracker2.import_queue) == 2  # Original + duplicate entry
            duplicate_entry = None
            for item in tracker2.import_queue:
                if item["status"] == "⏸️ Skipped" and item["detector"] == "Duplicate":
                    duplicate_entry = item
                    break
...
if __name__ == "__main__":
    ...
    try:
        ...
        tracker = ImportStatusTracker()
        print(f"✅ Tracker created with {tracker.orchestrator.plugin_manager.get_plugin_count()} plugins")
        
        print("Testing status mapping...")
        status = ImportStatusTracker._map_plugin_status_to_ui(PluginImportStatus.SUCCESS, "DefaultPlugin")
